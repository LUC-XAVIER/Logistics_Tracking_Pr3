...
public class TripService {
        ...
        public List<TripResponse> getTripsByAgent(String agentId, TripStatus status) {
                List<DriverTrip> trips = (status != null)
                                ? tripRepository.findByDriverIdAndStatus(agentId, status)
                                : tripRepository.findByDriverId(agentId);
                return trips.stream().map(this::mapToResponse).collect(Collectors.toList());
        }

        @Transactional
        public TripResponse createTrip(TripRequest request) {
                AgencyCoordinatesResponse sourceCoords = parcelServiceClient
                                .getAgencyCoordinates(request.getSourceAgencyId());
                AgencyCoordinatesResponse destCoords = parcelServiceClient
                                .getAgencyCoordinates(request.getDestAgencyId());
                ...
                DriverTrip trip = DriverTrip.builder()
                                ...
                                .sourceAgencyName(request.getSourceAgencyName())
                                .destAgencyId(request.getDestAgencyId())
                                .destAgencyName(request.getDestAgencyName())
        ...
        public TripResponse startTrip(UUID tripId) {
                ...
                kafkaEventPublisher.publishTripStarted(event);

                log.info("Trip started: id={}, parcels={}", tripId, parcelIds.size());
        ...
        private TripResponse mapToResponse(DriverTrip trip) {
                ...
                return TripResponse.builder()
                                ...
                                .driverId(trip.getDriverId())
                                .sourceAgencyId(trip.getSourceAgencyId())
                                .sourceAgencyName(trip.getSourceAgencyName())
                                .destAgencyId(trip.getDestAgencyId())
                                .destAgencyName(trip.getDestAgencyName())
                                .totalDistanceKm(trip.getTotalDistanceKm())
        ...
        public AgentStats getAgentStats(String agentId) {
                List<DriverTrip> allTrips = tripRepository.findByDriverId(agentId);

                double totalDistance = allTrips.stream()
                                .filter(t -> t.getStatus() == TripStatus.COMPLETED)
                                .mapToDouble(DriverTrip::getTotalDistanceKm)
                                .sum();

                long totalParcelsDelivered = allTrips.stream()
                                .filter(t -> t.getStatus() == TripStatus.COMPLETED)
                                .mapToLong(t -> tripParcelRepository.findByTripId(t.getId()).size())
                                .sum();

                return AgentStats.builder()
                                .totalTrips(allTrips.size())
                                .activeTrips(allTrips.stream().filter(t -> t.getStatus() == TripStatus.ACTIVE).count())
                                .completedTrips(allTrips.stream().filter(t -> t.getStatus() == TripStatus.COMPLETED)
                                                .count())
